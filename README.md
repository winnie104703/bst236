...
- **Local preview:** open `docs/index.html` in your browser to preview the homepage.
- **Publish on GitHub Pages:** push the repository and set GitHub Pages source to `main /docs` in the repository settings. The site will be available at:

	`https://<github-username>.github.io/<repository>/`

Replace `<github-username>` and `<repository>` with your GitHub username and repo name.

Links added in `docs/`:

- `docs/index.html` — homepage
- `docs/pacman.html` — Pac-Man game page (stub)
- `docs/arxiv.html` — arXiv feed page (stub)

Next: implement the Pac-Man game at `docs/pacman.html` and the nightly arXiv update workflow under `.github/workflows/` to refresh `docs/arxiv.html` or a data file used by that page.
